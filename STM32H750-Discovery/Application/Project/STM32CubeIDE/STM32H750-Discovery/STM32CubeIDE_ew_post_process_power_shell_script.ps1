...
# Check project id
If (-Not (Select-String -Path $Path -Pattern "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.306573613" -SimpleMatch -Quiet))
{
...
######################### EXCLUDES FROM BUILD ############################
$target = (((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.306573613"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).sourceEntries.entry).excluding
...
if($CurrentColorFormat -ne $NewColorFormat)
{
    ...
    (((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.306573613"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).sourceEntries.entry).excluding = $Newtarget
}
